...
newapp2.get('/approve', (req, res) => {
    ...
    connection.query(
        ...
        (err, results) => {
            ...
            // Insert the property into the all_properties table
           const sqlInsertSell = `
...
newapp2.get('/property-detail', (req, res) => {
  ...
  connection.query("SELECT * FROM all_properties WHERE id = ?", [propertyId], (err, results) => {
...
newapp2.post('/detail-contact', ensureAuthenticated, (req, res) => {
    console.log('Detail contact route hit - starting processing');
    
    const { name, email, phone, message } = req.body;
    const subject = "Property Enquiry";
    ...
    // Email options
    const mailOptions = {
        from: `"IBA Real Estate" <noreply@ibarealestate.com>`,  // Fixed sender
        to: 'admin@ibarealestate.com',  // Your admin/agent email
        subject: `Property Enquiry: ${subject}`,
        html: `
            <h2>New Property Enquiry from IBA REAL ESTATE Website</h2>
            <p><strong>Name:</strong> ${name}</p>
            <p><strong>Email:</strong> ${email}</p>
            <p><strong>Phone:</strong> ${phone}</p>
            ...
            <p>This is from the property detail contact form.</p>
        `
    };
    
    console.log('Starting email send for property enquiry...');
    
    // Setup timeout ID for clearing
    const timeoutId = setTimeout(() => {
        console.log('Email send timeout - forcing error response');
        res.status(500).json({ success: false, message: 'Email service timeout. Please try again.' });
    }, 30000);
    
    // Send email
    transporter.sendMail(mailOptions, (error, info) => {
        clearTimeout(timeoutId);  // <-- FIX: Clear timeout on response (success or error)
        ...
        console.log('Property enquiry email sent successfully:', info.response);
        return res.status(200).json({ success: true, message: 'Your enquiry has been sent to the agent! We\'ll get back to you soon.' });
    });
});
        ...
        (err, results) => {
            ...
            const insertQuery = `
                INSERT INTO sold_properties (
                    ...
                    image_data, video, amount, title, rentSell, \`property-type\`
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `;
